...
As we have seen when talking about [[Threads|threads]] they allow us to share these resources but they don't ensure that we have correct communication and synchronization among threads.
...
In this way everytime a task wants to access a shared object it must:
1. Aquire the lock of the shared object (and wait if it's not available)
...
Still the task busy-waits for the lock even with this method.
