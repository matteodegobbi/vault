...
# Prediction 
In the full RL problem we cannot apply the formulation we developed in the introduction directly since we don't have access to the true value function $v_\pi$.  
Instead later we will use as targets the sample return as in MC, the TD target as in TD(0) or the $\lambda$-return as in $\text{TD}(\lambda)$. For now let's stick to the true $v_\pi$ as the target for the these initial analysis.

An important difference with the tabular case is that we cannot aim to match the exact value function as this might not be reachable with the hypothesis class we chose to use to approximate the value function (like when in ML we relaxed the realizability assumption).

Theoretically we need to define a state distribution $\mu(s)\ge 0$, $\sum\limits_{s\in\cal S}{\mu(s)}=1$ to capture how much we care about each state based on the probability that the agent will be in that state: errors in more common states will be weighted more than in rare states.

Then we can define the objective function called the Mean-Squared Value Error (VE):
![[Pasted image 20251113140336.png]]

To simplify the analysis from now on we will refer only to the on-policy approach with continuing tasks, with these assumptions the state distribution is the stationary distribution under policy $\pi$.
...
Also in the following we will only show the formulas for value function approximation that is linear wrt the features, in the algorithms' pseudocode we don't assume linear approximation, instead we write the generic version using $\nabla\hat v(S,\textbf(w))$ (or $\nabla\hat q(S,A,\textbf(w))$ in the case of control algorithms).

![[Pasted image 20251115215821.png]]
(Tabular RL can be seen as a special case of linear approximation where the feature vector is a 1-hot vector with all the states)
