...
---
# $\text{TD}(\lambda)$
The idea is to use all the n-steps targets between TD(0) and Monte Carlo and use a weighted sum of these TD(n) approaches to update the value function.
...
Since in the weighted sum we need to use all the n-steps returns we can only compute $G^\lambda$ at the end of the episode, like we did in MC so we cannot learn along the way like we did in TD(0) and even TD(n).
...
Eligibility traces are a concept used for credit assegnment of the reward to different states visited during the trajectory, eligibility traces include both a recency heuristic (states near the reward get more credit) and a  frequency heuristic (a state that appears more often should get more credit).
