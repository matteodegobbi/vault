...
Another important advantage is that with policy parametrization we can choose functional forms of the policy based on the prior knowledge we have of the environment.
...
TODO controlla in the episodic case it's proportional while in the continuing case it's an equality
...
TODO aggiungi questione di dividere nabla pi per pi per normalizzare in base alla frequenza con cui e' presa l'azione
...
1. The update increases the parameter vector in the gradient's direction proportionally to the return $G_t$, makes sense as we want to move the parameter the most to directions where the return is the highest
