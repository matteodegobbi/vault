...
Example: $h{SLD}(n)$ is admissable (never overestimates the actual road distance)

Theorem: If h is admissible, A* using TREE-SEARCH is optimal
### Consistent heuristic

A heuristic h is consistent if for every node n, for every successor n' of n generated by an action a, h(n) ≤ c(n, a, n') + h(n’)
